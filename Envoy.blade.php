@setup
    ...
    exec('git log --oneline --format=%H:::%s | head -n 1', $log);
    list($hash, $message) = explode(':::', $log[0]);

    $deployMessage = sprintf(
        "Deployed commit %s \"%s\" to %s",
        mb_substr($hash, 0, 8),
...
@finished
	@telegram($telegramBot, $telegramChat, $deployMessage)
@endfinished
